...
@section("content")
    <div id="product-info">
        <div class="container">
            <h1>{{ __("Массажное кресло") }} <span>{{ $chair->device_id }}</span></h1>
            @if($status || $status === 0)
                <div class="status">
                    <div>
                        {{ __("Статус") }}:
    ...
    @if($status === 0)
        <div id="info-text" class="info-price info-bg">
            <div class="container">
                <p>{{ __("Выберите длительность массажа") }}</p>
                @include("rates")
            </div>
        </div>
    @endif
    ...
    @if($status === 3)
        <div id="busy-info" class="info-bg">
            <div class="container">
                <p>{{ __("Пожалуйста, дождитесь окончания работы кресла и обновите страницу") }}</p>
                <p><a href="{{ route("chair.show", ["deviceId" => $chair->device_id, "lang" => app()->getLocale()]) }}" class="btn">{{ __("Обновить страницу") }}</a></p>
    ...
    @if($status === null)
        <div id="busy-info" class="info-bg">
            <div class="container">
                <p>{{ __("Не удалось получить статус кресла, пожалуйста, обновите страницу") }}</p>
                <p><a href="{{ route("chair.show", ["deviceId" => $chair->device_id, "lang" => app()->getLocale()]) }}" class="btn">{{ __("Обновить страницу") }}</a></p>
                <div class="feedback-wrapper">
                    <p>{{ __("Заметили техническую неисправность?") }}<br> {{ __("Сообщите нам по телефону") }}:</p>
                    <p><a href="tel:+371 267 467 04" class="phone"><svg width="19" height="19" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg">
